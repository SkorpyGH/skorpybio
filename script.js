const CONFIG = {
  nickname: "Skorpy",
  avatarUrl: "avatar.png",        // <-- под твой HTML (avatar.png)
  ...
  buttons: [
    { text: "Раздачи ресов и общение", url: "https://discord.gg/Uzyc9wmN5m" },
...
const ICONS = {
  youtube:
    '<path d="M21.58 5.49a2.7 2.7 0 0 0-1.9-1.9C18 3.2 12 3.2 12 3.2s-6 0-7.68.4a2.7 2.7 0 0 0-1.9 1.9C2 7.17 2 10.67 2 10.67s0 3.5.42 5.18a2.7 2.7 0 0 0 1.9 1.9c1.68.39 7.68.39 7.68.39s6 0 7.68-.39a2.7 2.7 0 0 0 1.9-1.9C22 14.17 22 10.67 22 10.67s0-3.5-.42-5.18zM9.6 14.07v-6.8l6.4 3.4-6.4 3.4z"/>',
  telegram:
    '<path d="M21.93 3.68c-.27-.95-1.18-1.39-2.22-1.05L2.56 9.17c-1.06.41-1.06 2.03.04 2.41l4.47 1.55 1.55 4.96c.16.52.83.65 1.21.29l2.25-2.15 4.67 3.44c.86.63 2.12.2 2.33-.86l3.87-14.2c.11-.42-.04-.85-.35-1.13zM10.87 13.5l-.64 2.9-1.07-3.41 9.87-6.86-8.16 7.37z"/>',
  tiktok:
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- СОЦСЕТИ: вставляем в .social-row (если есть) ---
  ...
  if (socialRow) {
    ...
    CONFIG.socials.forEach((s) => {
      ...
      const svgContent = ICONS[s.icon] || "";
      a.innerHTML = `<svg viewBox="0 0 24 24">${svgContent}</svg>`;
      socialRow.appendChild(a);
    });
  }

  // --- КНОПКИ: если хочешь генерить из CONFIG, раскомментируй ---
  // const buttonsWrap = document.querySelector(".action-buttons");
  // if (buttonsWrap) {
  ...
  //     a.rel = "noopener";
  //     a.className = "big-btn";
  //     a.innerHTML = `<span>${b.text}</span><span class="arrow">→</span>`;
  ...
  function enterSite() {
    if (!startScreen || !mainContent) return;
  ...
  // если в HTML есть onclick="enterSite()"
  window.enterSite = enterSite;

  // если хочешь ещё и через addEventListener:
  if (startScreen) {
